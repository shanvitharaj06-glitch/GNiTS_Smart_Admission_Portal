...
// FETCH STUDENT DATA
$result = $conn->query("SELECT * FROM oci WHERE id=$id");
...
if ($action == "approve") {
    ...
    $update = $conn->query("UPDATE oci SET status='$status', allotted_branch='$branch' WHERE id=$id");
    ...
    $update = $conn->query("UPDATE oci SET status='$status', allotted_branch=NULL WHERE id=$id");
