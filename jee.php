...
<html>
<head>
<title>GNITS JEE Admission</title>
...
<h2>Application for Admission into First Year B.Tech under Category-B (JEE Mains)</h2>
...
<hr>
<h3>Entrance Details</h3>

<!-- NEW: JEE Hall Ticket -->
<label style="text-align:left;">JEE Hall Ticket Number <span class="required">*</span></label>
<input type="text" name="jee_hall_ticket" required>

<label style="text-align:left;">All India Rank in JEE (Main) 2023 <span class="required">*</span></label>
<input type="number" name="jee_rank" required>

<label style="text-align:left;">Percentile in JEE (Main) 2023 <span class="required">*</span></label>
<input type="number" step="0.01" name="jee_percentile" required>

<!-- NEW: EAMCET Hall Ticket -->
<label style="text-align:left;">TS-EAMCET Hall Ticket Number</label>
<input type="text" name="eamcet_hall_ticket">

<label style="text-align:left;">Rank in TS-EAMCET 2023</label>
...
onchange="validateFile(this,200,['image/jpeg','image/jpg','image/png'],'Photo')">
...
onchange="validateFile(this,300,['application/pdf'],'10th Memo')">
...
onchange="validateFile(this,300,['application/pdf'],'Intermediate Memo')">

<label style="text-align:left;">JEE Rank Card (PDF Max 300KB) <span class="required">*</span></label>
<input type="file" name="jee_card" required accept="application/pdf"
onchange="validateFile(this,300,['application/pdf'],'JEE Rank Card')">
...
onchange="validateFile(this,300,['application/pdf'],'EAMCET Rank Card')">
...
onchange="validateFile(this,200,['image/jpeg','image/jpg','image/png'],'Student Signature')">
...
onchange="validateFile(this,200,['image/jpeg','image/jpg','image/png'],'Parent Signature')">
...
<div class="button-group">
    ...
    <!-- Preview Button -->
    <button type="submit" name="preview" formaction="preview.php" class="preview-btn">
        Preview Before Pay
    </button>
    ...
    <button type="submit" name="payment" formaction="jee_submit.php" class="submit-btn">
        Pay and Submit
    </button>
...
document.querySelector("form").addEventListener("submit", function(e) {
    ...
    let confirm = document.querySelector("input[name='confirm_email']").value.trim();
    ...
    // ==========================
    function checkFile(inputName, maxSizeKB, allowedTypes, label){
        let file = document.querySelector("input[name='"+inputName+"']").files[0];

        if(!file) return true; // optional fields allowed

        let sizeKB = file.size / 1024;
        let type = file.type;

        if(sizeKB > maxSizeKB){
            alert(label + " must be less than " + maxSizeKB + "KB");
            return false;
        }

        if(!allowedTypes.includes(type)){
            alert(label + " has invalid file type");
            return false;
        }
    ...
    // ==========================
    if (
        !checkFile("photo", 200, ["image/jpeg","image/jpg","image/png"], "Photo") ||
        !checkFile("tenth", 300, ["application/pdf"], "10th Memo") ||
        !checkFile("inter", 300, ["application/pdf"], "Intermediate Memo") ||
        !checkFile("jee_card", 300, ["application/pdf"], "JEE Card") ||
        !checkFile("eamcet_card", 300, ["application/pdf"], "EAMCET Card") ||
        !checkFile("stud_sign", 200, ["image/jpeg","image/jpg","image/png"], "Student Signature") ||
        !checkFile("parent_sign", 200, ["image/jpeg","image/jpg","image/png"], "Parent Signature")
    ) {
        e.preventDefault();
        return;
    }
...
function validateFile(input, maxKB, allowedTypes, label) {
