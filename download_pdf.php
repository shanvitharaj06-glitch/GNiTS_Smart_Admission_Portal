<?php
require(__DIR__ . '/lib/FPDF-master/fpdf.php');
function docPair($pdf, $title1, $file1, $title2, $file2, &$y, $upload_dir){
    ...
    $url1 = $upload_dir . $file1;
    $url2 = $upload_dir . $file2;
...
function generateJeePdf($id,$conn){
     $id = (int)$id;
...
$row = $res->fetch_assoc();
 ...
 $upload_dir = __DIR__ . "/uploads/";
    $fileName = $row['reference_no'] . "_JEE.pdf";
...
function generateNriPdf($id,$conn){
    $id = (int)$id;
 
$res = $conn->query("SELECT * FROM nri WHERE id=$id");
$row = $res->fetch_assoc();

 
 $upload_dir = __DIR__ . "/uploads/";
    $fileName = $row['reference_no'] . "_NRI.pdf";
    $pdf_path = $upload_dir . $fileName;
...
$pdf->SetFont('Arial','B',10);
$pdf->SetXY(10, 38);
$pdf->Cell(100, 5, "NRI ID: " . $row['reference_no'], 0, 0);
$pdf->Cell(0, 5, date("F d, Y"), 0, 1, 'R');

// ================= TITLE =================
$pdf->SetFont('Arial','B',12);
$pdf->SetXY(15, 45);
$pdf->Cell(0, 5, "Application for Admission into first year B.Tech course under Category-B (NRI/NRI Sponsered)", 0, 1, 'C');
$pdf->Cell(0, 5, "for the A.Y 2025-26", 0, 1, 'C');
...
$pdf->SetFont('Arial','B',9);
$pdf->SetXY(10, $y);
$pdf->Cell(65, 7, "Relationship");
$pdf->SetFont('Arial','',9);
$pdf->SetXY(75, $y);
$pdf->Cell(25, 7, $row['relationship']);
$y += 8;

$pdf->SetFont('Arial','B',9);
$pdf->SetXY(10, $y);
$pdf->Cell(65, 7, "Sponsor Name");
$pdf->SetFont('Arial','',9);
$pdf->SetXY(75, $y);
$pdf->Cell(25, 7, $row['sponsor_name']);
...
$pdf->SetFont('Arial','',9);
$pdf->SetXY(155, $y);
$pdf->Cell(35, 7, $row['place_country']);
$y += 8;
labelValueRow($pdf, "Rank in TGEAPCET - 2025", $row['eamcet_rank'], $y);
...
docPair($pdf, "10th Marks Memo", $row['tenth'], "Intermediate / 12th Marks Memo", $row['inter'], $y, $upload_dir);
docPair($pdf, "Passport/VISA of NRI", $row['passport'], "TGEAPCET 2025 Rank Card (optional)", $row['eamcet_card'], $y, $upload_dir);
docPair($pdf, "Address proof/Driving license of NRI", $row['nri_driving'], "NRI Sponsership letter", $row['nri_letter'], $y, $upload_dir);
// ==================== PAGE 2 ====================
$pdf->AddPage();
...
//oci pdf
function generateOciPdf($id, $conn)
{
 $id = (int)$id;
...
$upload_dir = __DIR__."/uploads/";
$fileName = $row['reference_no'] . "_OCI.pdf";
    $pdf_path = $upload_dir . $fileName;
